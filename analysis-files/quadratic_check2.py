...
hists_acc=[]
for i in range(0, 11):
	aos=yoda.read(sys.argv[1]+str(i)+'.yoda', asdict=False)

	#grabs all H_PT histograms (all the reweightings)

	for hist in aos:
		if hist.path.startswith("/SimpleHiggs/H_PT[Weight_MERGING"):
			hists.append(hist)
		if hist.path.startswith("/SimpleHiggs/acc_H_PT[Weight_MERGING"):
